    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">Manage awards, hackathon recognitions, and paper publications.</p>
        </div>
      ...
      {isEditing ? (
        <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4">
          <h3 className="text-base font-bold">Edit Achievement</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold mb-1">Category Badge Text</label>
          ...
          <div>
            ...
            <textarea rows={3} value={current.description} onChange={e => setCurrent({ ...current, description: e.target.value })} className="w-full p-2.5 rounded-xl border bg-slate-50 dark:bg-slate-700 text-sm" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-ieee-blue">Save</button>
          </div>
        </form>
      ) : (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden divide-y divide-slate-100 dark:divide-slate-700">
          {achievements.map(a => (
            <div key={a.id} className="p-4 flex items-center justify-between gap-4">
              <div>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded text-white bg-ieee-blue uppercase">{a.category}</span>
